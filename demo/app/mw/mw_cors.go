...
func Cors(c *gin.Context) {
	...
	if origin != `` {
		...
		c.Header(`Access-Control-Allow-Methods`, "POST,GET,OPTIONS,PUT,DELETE,UPDATE,PATCH,HEAD,TRACE,CONNECT") // 服务器支持的所有跨域请求的方法,为了避免浏览次请求的多次'预检'请求
		...
		//c.Header("Access-Control-Max-Age", "3600")           // 缓存请求信息 单位为秒
		c.Header(`Cache-Control`, `no-store`)                // 不做任何接口缓存
